...
class Train:
    ...
    def __str__(self):
        return f'Электичка типа "{self.train_type}" со станции {self.from_station} отправляется {self.departure.time()} и прибывает на {self.to_station} в {self.arrival.time()}'